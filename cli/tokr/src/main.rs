//! `tc` (Token Compiler) is the CLI frontend for Tokr.
//!
//! It natively parses `tokr.json` configurations, maps CLI flags (like `--watch` and `--check`),
//! and hooks into `tokr-driver` to orchestrate end-to-end token compilation.
...
fn main() {
    ...
    let config = if config_path.exists() {
        ...
        // default empty config, overridden by cli
        TokrConfig {
            schema: None,
            input: Vec::new(),
            output: None,
    ...
    // We treat the directory of `tokr.json` as the root directory, or cwd if default
    ...
    if cli.watch {
        ...
        for res in rx {
            match res {
                Ok(event) => {
                    // Only recompile on modify or create
                    if matches!(
                        event.kind,
